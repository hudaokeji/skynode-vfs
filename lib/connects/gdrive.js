...
*/
    ...
    async mkdir(path) {
        ...
        const result = await grest.createFile(this.session.credentials, parentFolder.id, pathname, grest.mimeTypeFolder);
    ...
    async readdir(path,options) {
        ...
        const readFiles = async params => {
            ...
            for(const file of result.files) {
                files.push(grest.toUnifiedFileInfo(file,path+"/"+file.name));
            }
    ...
    async readFile(path) {
        ...
        return await grest.downloadFile(this.session.credentials,file.id);
    }
    ...
    async rmdir( path) {
        ...
        if (dir.id === await grest.getActualRootFolderId(this.session.credentials))
        {
    ...
    async stat(path) {
        ...
        if(path === "/") {
            ...
            return grest.toUnifiedFileInfo(res.data,path);
        }
        const parentFolder = await this.getFileOfPath(absPath.getPathPart());
    ...
    async writeFile( path, data,mimeType) {
        ...
        if(files.length === 0) {
            const file = await grest.createFile(this.session.credentials,parentFolder.id, filename, mimeType);
            const result = await grest.updateFile(this.session.credentials,file.id, mimeType, data);
            ///if(parentFolder.id === this.getCurrentFolderId()) {
            ///    await this.fireCwdUpdate();
            ///}
            return result;
