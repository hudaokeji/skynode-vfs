...
class LocalConnector {
  ...
  find(path,options) { //TODO
    ...
    return nfs.find(realPath,options);
  }
  ...
  mkfile(path) {
    const realPath = this.toRealPath(path);
    try {
      return nfs.ensureFileAsync(realPath);
    } catch (e) {
      return Promise.reject(e);
    }
  ...
  writeFile( fileId, data) { // 
    ...
    return nfs.writeFileAsync(realPath, data).then(()=>{
        return this.stat(fileId);
    });
  }
